...
int returnbook(const char *username, const char *returnisbn) {
    ...
    if (!found_in_issued) {
        // printf("Error: No record found for user '%s' with ISBN '%s' in issued.csv.\n", username, returnisbn);
...
int main() {
    ...
    char returnisbn[MAX_ISBN_LENGTH];

    printf("Enter username: ");
    scanf("%50s", username);
    printf("Enter ISBN of the book to return: ");
    scanf("%10s", returnisbn);

    if (returnbook(username, returnisbn) == 0) {
